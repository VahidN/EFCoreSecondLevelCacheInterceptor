...
public class SecondLevelCacheInterceptorInvalidationTests
{
    ...
    [DataRow(TestCacheProvider.EasyCachingCoreRedis)]
    public void
        TestInsertingDataToOtherTablesShouldNotInvalidateTheCacheDependencyAutomatically(
    ...
    [DataRow(TestCacheProvider.EasyCachingCoreRedis)]
    public void
        TestInsertingDataToRelatedTablesShouldInvalidateTheCacheDependencyAutomatically(TestCacheProvider cacheProvider)
        => EFServiceProvider.RunInContext(cacheProvider, LogLevel.Debug, cacheAllQueries: false,
            (context, loggerProvider) =>
            {
                var isActive = true;
    ...
    public void TestRemoveTptDataShouldInvalidateTheCacheAutomatically(TestCacheProvider cacheProvider)
        => EFServiceProvider.RunInContext(cacheProvider, LogLevel.Debug, cacheAllQueries: false,
            (context, loggerProvider) =>
            {
                var list1 = context.Posts.OfType<Page>()
                    .Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
                    .ToList();

                Assert.AreEqual(expected: 0, loggerProvider.GetCacheHitCount());

                var list2 = context.Posts.OfType<Page>()
                    .Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
                    .ToList();
                ...
                var list3 = context.Posts.OfType<Page>()
                    .Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
                    .ToList();
        ...
        => EFServiceProvider.RunInContext(cacheProvider, LogLevel.Debug, cacheAllQueries: false,
            (context, loggerProvider) =>
            {
                ...
                if (!context.Users.Any(user => user.Name == user1Name))
                {
                    ...
                    {
                        Name = user1Name,
                        AddDate = new DateTime(year: 2020, month: 4, day: 3, hour: 17, minute: 50, second: 39,
                            millisecond: 503),
                        UpdateDate = null,
                        Points = 1000,
                        IsActive = true,
                        ByteValue = 1,
                        CharValue = 'C',
                        DateTimeOffsetValue = new DateTimeOffset(new DateTime(year: 2020, month: 4, day: 3, hour: 17,
                            minute: 50, second: 39, millisecond: 503)),
                        DecimalValue = 1.1M,
                        DoubleValue = 1.3,
                        FloatValue = 1.2f,
                        GuidValue = new Guid(g: "236bbe40-b861-433c-8789-b152a99cfe3e"),
                        TimeSpanValue = new TimeSpan(days: 1, hours: 0, minutes: 0, seconds: 0, milliseconds: 0),
                        ShortValue = 2,
                        ByteArrayValue = [1, 2],
                        UintValue = 1,
                        UlongValue = 1,
                ...
                var p98 = context.Products.Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
                ...
                p98 = context.Products.Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
                    .FirstOrDefault(p => p.ProductId == product.ProductId);
    ...
    public void TestCachingByteArrays(TestCacheProvider cacheProvider)
        => EFServiceProvider.RunInContext(cacheProvider, LogLevel.Debug, cacheAllQueries: false,
            (context, loggerProvider) =>
            {
                ...
                var userData = context.Users.Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
                ...
                userData = context.Users.Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
                    .FirstOrDefault(p => p.Id == user1.Id);
    ...
    [DataRow(TestCacheProvider.EasyCachingCoreRedis)]
    public void
        TestInsertingMultipleDataIntoTheSameTableShouldInvalidateTheCacheAutomatically(TestCacheProvider cacheProvider)
        => EFServiceProvider.RunInContext(cacheProvider, LogLevel.Debug, cacheAllQueries: false,
            (context, loggerProvider) =>
            {
                var isActive = true;
