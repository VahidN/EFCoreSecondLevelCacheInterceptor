...
public class HomeController : Controller
{
    ...
    {
        var count = await _context.Posts.Where(x => x.Id > 0)
            .Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
            .CountAsync();
    ...
    {
        var count = await _context.Posts.Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
            .CountAsync(x => x.Id > 0);
    ...
    public async Task<IActionResult> CollectionsTest()
    {
        ...
        var post1 = await _context.Posts.Where(x => collection1.Contains(x.Id))
            .Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
            .FirstOrDefaultAsync();
        ...
        var post2 = await _context.Posts.Where(x => collection2.Contains(x.Id))
            .Cacheable(CacheExpirationMode.Absolute, TimeSpan.FromMinutes(minutes: 45))
            .FirstOrDefaultAsync();
